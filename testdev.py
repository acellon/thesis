...
def scratch_net(input_var):
    ...
    net['conv1'] = layers.Conv2DLayer(net['data'],  num_filters=4,
                                      filter_size=(1, 7), pad='same', nonlinearity=rectify)
...
def scratch_model(input_var, target_var, net):
    ...
    test_acc = T.mean(lasagne.objectives.binary_accuracy(
        test_prediction, target_var), dtype=theano.config.floatX)
...
ax2 = ax1.twinx()
ax2.plot(range(num_epochs), np.asarray(val_acc) *
         100, color='g', label='Validation accuracy')
